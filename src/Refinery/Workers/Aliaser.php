    ...
    public function work()
    {
        if (file_exists($d = $this->get('directory', false)) && file_exists($o = $this->get('output', false))) {
            if (($file = fopen($o, 'w', false)) !== false) {
                 ...
                 * @var \SplFileInfo $lib
                 */
                foreach ($this->getIterator($d) as $lib) {
                    $i++;
                    ...
                    $contents = file_get_contents($lib->getPathname());

                    if (preg_match("/namespace ([a-z_\\\\]+);/i", $contents, $match)) {
                        $namespace = str_replace('\\', '\\\\', $match[1]) . '\\\\' . $class;
                    } else {
                        $namespace = $class;
                    }

                    $lines[] = "    '{$class}' => '{$namespace}',\n";
                ...
                $this->farewell = "I aliased {$i} classes";
